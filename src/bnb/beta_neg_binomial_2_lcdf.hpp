#ifndef STAN_MATH_PRIM_PROB_BETA_NEG_BINOMIAL_2_LCDF_HPP
#define STAN_MATH_PRIM_PROB_BETA_NEG_BINOMIAL_2_LCDF_HPP
...
inline stan::return_type_t<T_mu, T_sigma, T_gamma> beta_neg_binomial_2_lcdf(const T_n& n, 
                                                    const T_mu& mu,
                                                    const T_sigma& sigma,
                                                    const T_gamma& gamma,
  ...
  static const char* function = "beta_neg_binomial_2_lcdf";
  ...
  for (size_t i = 0; i < max_size_seq_view; i++) {
    ...
    T_partials_return lgamma_r_dbl = lgamma(r_dbl);
    T_partials_return lgamma_beta_dbl = lgamma(beta_dbl);
    T_partials_return lgamma_alpha_dbl = lgamma(alpha_dbl);
    T_partials_return lgamma_r_alpha_dbl = lgamma(r_dbl + alpha_dbl);
    T_partials_return lgamma_beta_alpha_dbl = lgamma(beta_dbl + alpha_dbl);

    VectorXd j_values = VectorXd::LinSpaced(n_dbl + 1, 0, n_dbl);
    VectorXd lgamma_r_j = (r_dbl + j_values.array()).unaryExpr([](double x) { return lgamma(x); });
    VectorXd lgamma_beta_j = (beta_dbl + j_values.array()).unaryExpr([](double x) { return lgamma(x); });
    VectorXd lgamma_j_1 = (j_values.array() + 1.0).unaryExpr([](double x) { return lgamma(x); });
    VectorXd lgamma_r_alpha_beta_j = (r_dbl + alpha_dbl + beta_dbl + j_values.array()).unaryExpr([](double x) { return lgamma(x); });
    VectorXd log_p = lgamma_r_j.array() + lgamma_beta_j.array() + lgamma_r_alpha_dbl + lgamma_beta_alpha_dbl
                    - lgamma_j_1.array() - lgamma_r_dbl - lgamma_beta_dbl - lgamma_alpha_dbl - lgamma_r_alpha_beta_j.array();
    T_partials_return CDF = log_p.array().exp().sum();


    P += log(CDF);
    const T_partials_return C = lgamma(r_plus_n+1.0) + lbeta(a_plus_r, b_plus_n+1.0) - lgamma(r_dbl) - lbeta(alpha_dbl, beta_dbl) - lgamma(n_dbl+2.0);
    const T_partials_return F = (1.0-CDF)/exp(C);
    const T_partials_return P_i = exp(C) * F;
    ...
    if (!is_constant_all<T_mu, T_sigma, T_gamma>::value) {
      grad_F32(dF, one, b_plus_n + 1, r_plus_n + 1, n_dbl + 2, a_plus_r + b_plus_n + 1, one, 1e-4, 100000);
    }
    ...
    if (!is_constant_all<T_mu>::value) {
      ...
      ops_partials.edge1_.partials_[i] += -g * P_i / (1 - P_i);
    }
    if (!is_constant_all<T_sigma>::value) {
      ...
      ops_partials.edge2_.partials_[i] += -g * P_i / (1 - P_i);
    }
    if (!is_constant_all<T_gamma>::value) {
      ...
      ops_partials.edge3_.partials_[i] += -g * P_i / (1 - P_i);
    }
  }
